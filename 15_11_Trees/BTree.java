...
class Solution {
    ...
    public int greatNodes(BinaryTreeNode root) {
        ...
        dfsHelper(root,root.data);
        return result;
    }
    public void dfsHelper(BinaryTreeNode root,int currentGreat){
        if(root==null){
            return;
        }
        if(root.data>=currentGreat){
            result++;
            dfsHelper(root.left, root.data);
            dfsHelper(root.right, root.data);
        }
        else{
            dfsHelper(root.left, currentGreat);
            dfsHelper(root.right, currentGreat);
        }
    }
